...
const { __subscriberRef } = require("scheduler/tracing");
...
//promise chaining => eg
//promise combinator
1.Promise.all => takes all promises and return result after all promises are executed
It returns true only if all promises are fulfilled , even if one promise fail it fails complete promise.all

2.Promise.race => returns first promise that gets resolved or rejected
3.Promise.allSettled => returns all promises with their state i.e fullfilled or rejected
4.Promise.any => same as promise.race but it only returns fst fullfilled prmse and ignores rejects once. If all promises were rejected then it gives all promises were rejected. 
...
//and it will sync code in it will execute when fun is called
start
middle
1
end
...
promise
  ...
  create a first promise , and create a 2nd promise that resolves first promise
